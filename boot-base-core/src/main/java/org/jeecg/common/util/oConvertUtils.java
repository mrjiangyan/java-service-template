	...
	public static String getRealIp() throws SocketException {
		...
		while (netInterfaces.hasMoreElements() && !finded) {
			...
			while (address.hasMoreElements()) {
                ...
                // 外网IP
				if (!ip.isSiteLocalAddress() && !ip.isLoopbackAddress() && !ip.getHostAddress().contains(COLON)) {
					netip = ip.getHostAddress();
					finded = true;
					break;
				} else if (ip.isSiteLocalAddress() && !ip.isLoopbackAddress() && !ip.getHostAddress().contains(COLON)) {
                    // 内网IP
