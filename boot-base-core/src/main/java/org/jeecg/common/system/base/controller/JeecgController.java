...
public class JeecgController<T, S extends IService<T>> {
  ...
  protected <R> ModelAndView  exportXlsFormList(HttpServletRequest request, List<R> exportList, Class<R> clazz, String title) {
    LoginUser sysUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();
    // Step.3 AutoPoi 导出Excel
    ModelAndView mv = new ModelAndView(new JeecgEntityExcelView());
    //此处设置的filename无效 ,前端会重更新设置一下
  ...
  protected <R> void  exportXlsToStream(OutputStream stream, List<R> exportList, Class<R> clazz, String title) throws IOException {
    LoginUser sysUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();
    // Step.3 AutoPoi 导出Excel
    //update-begin--Author:liusq  Date:20210126 for：图片导出报错，ImageBasePath未设置--------------------
    ExportParams  exportParams = new ExportParams(title, "导出人:" + sysUser.getRealname(), title);
     ...
     * @return
     */
    protected Result<?> importExcel(HttpServletRequest request, Class<T> clazz) {
